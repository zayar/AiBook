...
import { salespersonAPI, CreateSalespersonRequest } from '@/lib/salesperson-api';
...
export default function NewSalespersonPage() {
  ...
  const [formData, setFormData] = useState<CreateSalespersonRequest>({
    name: '',
    email: '',
    phone: '',
    department: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await salespersonAPI.createSalesperson(submitData);
      console.log('Salesperson created:', response);
      router.push('/salespeople');
    } catch (error: any) {
      console.error('Error creating salesperson:', error);
      alert(error.message || 'Failed to create salesperson');
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-2">Add a new member to your sales team</p>
            </div>
          </div>
        </motion.div>
