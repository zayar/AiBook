...
async function cleanKeepEssentials() {
  try {
    console.log('🧹 Cleaning database, keeping only tenants and chart of accounts...');
    ...
    try {
      ...
      console.log('⚠️  Some COGS/inventory tables might not exist, continuing...');
    }
    ...
    try {
      ...
      console.log('✅ Deleted all AI-related data');
    } catch (error) {
      console.log('⚠️  Some AI tables might not exist, continuing...');
    }
    ...
    if (accountsByType.length > 0) {
      ...
      accountsByType.forEach(({ type, _count }) => {
